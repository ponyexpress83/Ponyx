  ...
  try {
    ...
    await prisma.aIMessage.createMany({
      data: [
        ...
        { role: "assistant", content: result.response, agent: "ANALYST", projectId },
      ],
    });

    if (result.score !== undefined) {
      const updateData: { score: number; pricingTier?: string } = { score: result.score };
      if (result.pricingTier) {
        updateData.pricingTier = result.pricingTier;
      }
